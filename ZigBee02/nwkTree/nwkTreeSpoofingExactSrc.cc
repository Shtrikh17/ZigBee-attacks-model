...
void nwkTreeSpoofingExactSrc::initialize(){
...
nwkPacketTree* nwkTreeSpoofingExactSrc::generateDataPacket(){
    ...
    if(current_spoof<spoof_period){
        ...
        srcAddress = intuniform(0, numRFDs);
        current_spoof++;
    }
    else{
        srcAddress = spoof_address;
        srcPANAddress = address_helper_module->getAddress(spoof_pan);
