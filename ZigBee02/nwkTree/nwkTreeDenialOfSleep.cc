...
void nwkTreeDenialOfSleep::initialize(){
    nwkTreeNormal::initialize();
    attack_address = par("attack_address");
    attack_pan = par("attack_pan");
}

nwkPacketTree* nwkTreeDenialOfSleep::generateDataPacket(){
    ...
    int destPANAddress = address_helper_module->getAddress(attack_pan);
