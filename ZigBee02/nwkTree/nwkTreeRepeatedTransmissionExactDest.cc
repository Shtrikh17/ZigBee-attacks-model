...
void nwkTreeRepeatedTransmissionExactDest::initialize(){
...
void nwkTreeRepeatedTransmissionExactDest::handleMessage(cMessage* msg){
    ...
    if(msg->isPacket()){
        ...
        if(!(strcmp(packet->getName(), "data"))){
            ...
            if(packet->getDestPANAddress()!=myPANAddress){
                ...
                if((packet->getDestAddress()==select_address)&&(packet->getDestPANAddress()==select_pan)){
                    current_select_number++;
                    if(current_select_number>=select_period){
                        repeat_queue.push_back(frame_sequence->dup());
                        current_select_number = 0;
                    }
                }
        ...
        if(!strcmp(msg->getName(), "repeat")){
            ...
            collectorModule->addForwardedPackets(address_helper_module->getIndex(myPANAddress));
            macFrameTree* to_repeat = repeat_queue.front();
            repeat_queue.erase(repeat_queue.begin());
            sendingProcedure(to_repeat);
        }
